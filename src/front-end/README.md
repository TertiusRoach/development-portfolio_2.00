...
This [Sass file](https://github.com/TertiusRoach/development-portfolio_2.00/blob/main/src/front-end/pages/override-bootstrap.scss) serves as an override mechanism, empowering the customization of targeted designs within the Bootstrap framework. With an already established foundational structure, it offers adaptability by allowing the modification of default values. Then there's another [SASS file](https://github.com/TertiusRoach/development-portfolio_2.00/blob/main/src/front-end/pages/index/index.scss) that only applies styles to the specific page it's situated in and encapsulates the default values for a company's registered colors and logos. Its scope isn't global like the aforementioned file. Additionally, it sets the page defaults for CSS margins and padding on HTML elements. I typically initialize these defaults to zero and subsequently tailor them to meet the unique design requirements of the specific page to not affect the entire project, unlike **override-bootstrap**, which is meant to change the entire scope of all the pages. The reason I use Bootstrap is because of its text scaling which they already figured out and refined over the years for different resolutions, no need to make another CSS framework when the roadblock was already overcame.
